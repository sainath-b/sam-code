...
```bash
sam-nodejs-multi-step$ sam build
```
...
```bash
sam-nodejs-multi-step$ sam logs -n StockCheckerFunction --stack-name sam-nodejs-multi-step --tail
```
...
```bash
sam-nodejs-multi-step$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name sam-nodejs-multi-step
```
